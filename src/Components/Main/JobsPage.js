...
import "../../assets/css/circular-container.css";
import "../../assets/css/developer.css";
import "animate.css";
import image_14 from "../../landingstatic/img/image 14.png"
...
import PreviousActive from "./HomePageComponents/PreviousActive";
import TodaysJobs from "./HomePageComponents/TodaysJobs";
...
const JobsPage = () => {
    ...
    // console.log("monthlyJobs",monthlyJobs);
    ...
    const initClientImages = async () => {
        try {
            ...
            setclientImgs(hotjobs.apiData ? JSON.parse(hotjobs.apiData) : []);
        } catch (error) {
            setclientImgs([]);
        }
    };
    const initDailyAndMonthlyJobs = async () => {
        try {
            ...
            //setdailyJobs([]);
            setmonthlyJobs(jobs.Table1.slice(0, 50));
        } catch (error) {
            setopenLoader(false);
            console.log(error);
        }
  ...
  animation: ${flip} 2s infinite;
    ...
    return (
        <>
            <Helmet>
                <title>HuntsJob - Todays Jobs - Previous Active Jobs - Trending Jobs - Popular Job Categories</title>
                <meta name="description" content="HuntsJob - Todays Jobs - Previous Active Jobs - Trending Jobs - Popular Job Categories - HuntsJob offers a comprehensive platform for job seekers. Discover today's latest job openings, browse through previously active listings, explore trending opportunities, and find your ideal role in our popular job categories. Start your job search with HuntsJob and take the next step in your career." />
            </Helmet>
            <Backdrop
            ...
            <>
                <Layout>
                    {
                        dailyJobs.length > 0 ?
                            <section id="idxTargetHotJOb" style={{ padding: "0" }}>
                                    ...
                                    <div>
                                        <div className="slider-container">
                                            <div class="section" style={{ paddingBottom: "50px" }}>
                                                ...
                                                <div class="heading">
                                                    <span style={{ color: "#FE5C35" }}>{t("Todays")} </span>
                                                    {t("Jobs")}
                                                </div>
                                            </div>
                                        </div>
                                        <TodaysJobs dailyJobs={dailyJobs}
                                            x={x}
                                    ...
                                    <div>
                                        <div className="slider-container">
                                            <div class="section" style={{ paddingBottom: "50px" }}>
                                                ...
                                                <div class="heading">
                                                    <span style={{ color: "#FE5C35" }}>{t("Previous")} </span>
                                                    {t("Active Jobs")}
                                                </div>
                                            </div>
                                        </div>

                                        <PreviousActive monthlyJobs={monthlyJobs}
                                            x={x}
                                            handleOnclickJobDetailsPage={handleOnclickJobDetailsPage}
                                            handleSaveJobClick={handleSaveJobClick}
                                            href={href} />
                    ...
                    <section className="section-companies mb-4">
                        <PopularCategories
